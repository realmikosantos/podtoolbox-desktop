/**
 * PodToolbox Desktop — Electron main process.
 *
 * Wraps podwires.com/podtoolbox in a native desktop window.
 *
 *   - Remembers window size & position between launches
 *   - Opens external (non-podwires.com) links in the user's default browser
 *   - Native app menu with back / forward / reload / zoom
...
const APP_URL        = 'https://podwires.com/podtoolbox/';
const APP_HOST       = 'podwires.com';
const APP_PATH       = '/podtoolbox/';
const PROTOCOL       = 'podtoolbox';
const USER_AGENT_TAG = 'PodToolboxDesktop';
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    minHeight: 600,
    show: false,
    backgroundColor: nativeTheme.shouldUseDarkColors ? '#1D1C2E' : '#f4f4f8',
    title: 'PodToolbox',
  ...
  // Internal = anywhere under podwires.com/podtoolbox/* (the PodToolbox sub-site).
  // Everything else (including other parts of podwires.com) opens externally.
  const isInternal = (url) => {
    try {
      const u = new URL(url);
      const hostOk = (u.hostname === APP_HOST || u.hostname.endsWith(`.${APP_HOST}`));
      return hostOk && u.pathname.startsWith(APP_PATH);
    } catch {
      return false;
    }
  };
  ...
  mainWindow.webContents.on('did-fail-load', (_e, errorCode, errorDescription, validatedURL) => {
    ...
    const html = `<!doctype html><meta charset="utf-8"><title>PodToolbox — offline</title>
      <style>
        body { font-family: -apple-system, Segoe UI, Roboto, sans-serif; background:#1D1C2E;
        ...
        button:hover { background:#6E63D9; }
        code { color:#9795b5; font-size:.8rem; }
      </style>
      <div class="card">
        <h1>Can't reach PodToolbox</h1>
...
function buildMenu() {
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Open PodToolbox in browser',
          click: () => shell.openExternal(APP_URL),
        },
        {
          label: 'Submit a Tool',
          click: () => mainWindow?.loadURL(`${APP_URL}submit/`),
        },
        {
        ...
        { type: 'separator' },
        {
          label: 'About PodToolbox Desktop',
          click: () => {
            dialog.showMessageBox(mainWindow, {
              type: 'info',
              title: 'PodToolbox Desktop',
              message: `PodToolbox Desktop v${app.getVersion()}`,
              detail: `Electron ${process.versions.electron}\nChromium ${process.versions.chrome}\nNode.js ${process.versions.node}\n\npodwires.com/podtoolbox`,
              buttons: ['OK'],
            });
          },
        },
